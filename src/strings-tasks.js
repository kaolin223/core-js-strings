...
function removeLastOccurrences(str, value) {
  ...
  const letterWithReplace = arr.replace(value, '');
  return letterWithReplace.split(' ').reverse().join(' ');
}
...
function sumOfCodes(str) {
  ...
  const arr = str.split('');
  const arrayNumbers = arr.map((char) => char.charCodeAt());
  const reduce = arrayNumbers.reduce(
...
function reverseWords(str) {
  const arr = str.split(' ');
  const map = arr.map((rever) => rever.split('').reverse().join(''));
  return map.join(' ');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (letter) => rot13[alphabet.indexOf(letter)]);
}
